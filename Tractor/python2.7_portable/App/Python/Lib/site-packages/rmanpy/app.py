...
class App(QtGui.QApplication):
    def __init__(self, argv):
        ...
        self.setApplicationName("rmanpy")
        self.setOrganizationName("renderman")
        self.setOrganizationDomain("com.pixar")
        self.m_apptree = os.path.normpath(self.applicationDirPath() + "/..")
        self.m_plugdir = os.path.join(self.m_apptree, "lib/Qt/plugins")
        self.m_rezdir = os.path.join(self.m_apptree, "lib/Resources")
        for defpath in self.libraryPaths():
            self.removeLibraryPath(defpath)
        self.addLibraryPath(self.m_plugdir)

        self.m_pixmaps = {}
        self.setWindowIcon(self.getPixmap('rmanpy.png'))
        kineticPix = self.getPixmap('kinetic.png')
        bgPix = self.getPixmap('Time-For-Lunch-2.jpg')
        ...
        ellipseButton = Button(self.getPixmap('ellipse.png'), buttonParent)
        figure8Button = Button(self.getPixmap('figure8.png'), buttonParent)
        randomButton = Button(self.getPixmap('random.png'), buttonParent)
        tiledButton = Button(self.getPixmap('tile.png'), buttonParent)
        centeredButton = Button(self.getPixmap('centered.png'), buttonParent)
        ...
        # Ui.
        view = View(scene)
        view.setWindowTitle("rmanpy dashboard")
    ...
    def getPixmap(self, nm):
        try:
            pm = self.m_pixmaps[nm];
        except KeyError:
            file = "%s/images/%s" % (self.m_rezdir, nm)
            pm = QtGui.QPixmap(file)
            self.m_pixmaps[nm] = pm
        return pm
