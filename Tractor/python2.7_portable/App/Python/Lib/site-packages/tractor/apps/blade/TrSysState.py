...
class TrSysState(object):
    ...
    def getWinPlatformFromRegistry (self):
	    ...
	    try:
                # for win10+
                major = _winreg.QueryValueEx(vkey, "CurrentMajorVersionNumber")
                minor = _winreg.QueryValueEx(vkey, "CurrentMinorVersionNumber")
    ...
    class TrGPUInfo (object):
        ...
        def ResolveGPUInfo (self, gpuEnumByPlatform={}, gpuExcludePatterns=[]):
            ...
            if not gpuExcludePatterns:
                gpuExcludePatterns = []
            elif type(gpuExcludePatterns) in (str,unicode):
        ...
        def resolveGPU_Linux (self, argv, exclusions):
            ...
            useShell = type(argv) in (str, unicode)
