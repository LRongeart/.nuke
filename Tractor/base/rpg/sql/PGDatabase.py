...
import tractor.base.rpg.sql.Database as Database
import tractor.base.rpg.sql.Join as Join
import tractor.base.rpg.sql.Function as Function
...
class PGDatabase(Database.Database):
    ...
    def open(self, timeout=0):
        ...
        try:
            ...
            raise Database.SQLConnectionError("unable to establish connection with database %s on " "%s as user '%s': %s" % (self.db, self.dbhost, self.user, str(err)))
