...
def date2secs(date):
    ...
    return (date - datetime.datetime(1970,1,1, tzinfo=tractor.dateutil.tz.tzutc())).total_seconds()
...
def hostSleep(factor):
    ...
    try:
        ...
        delay_unit = int(hostname[-2:])
    except ValueError:
        delay_unit = 1

    delay = delay_unit * factor
    logging.info(f"[hostSleep] Host '{hostname}' sleeping for {delay} seconds")
    time.sleep(delay)
