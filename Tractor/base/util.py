...
class UnknownColor(Exception):
    """Raised when an invalid foreground or background color is used."""
    pass
...
class TerminalColor:
    ...
    def __init__(self, fg, bg=None):
        if fg not in self.colorcodes['fg']:
            raise UnknownColor("'%s' is an unknown foreground color, must be one of %s"
                               % (fg, self.colorcodes['fg'].keys()))
        self.foreground = fg
        if bg and bg not in self.colorcodes['bg']:
            raise UnknownColor("'%s' is an unknown background color, must be one of %s"
                               % (bg, self.colorcodes['bg'].keys()))
        self.background = bg
