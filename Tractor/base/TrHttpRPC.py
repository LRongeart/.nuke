...
import tractor.base.urllib2 as urllib2
import struct, types
...
class TrHttpRPC (object):
    ...
    def Transaction (self, tractorverb, formdata, parseCtxName=None,
        ...
        try:
            ...
            print(f"[Tractor] Transaction failed with: {type(e).__name__}: {e}")
            errcode = getattr(e, "code", 500)
            errmsg = str(e)
            print(errmsg)
...
#!!! DISABLE WHEN RUNNING LIVE !!!#
#-------------------------------------------------------------------------------------------------------------------#
    def trSitePasswordHash(user, passwd): #!!! DISABLE WHEN RUNNING LIVE !!!#
        return None  # or some fake hash like "abc123" if needed # !!! DISABLE WHEN RUNNING LIVE !!!#
#-------------------------------------------------------------------------------------------------------------------#
#///////
   

    def PasswordRequired(self):
        if self.passwdRequired is not None:
        ...
        try:
            ...
            if data and not err:
                namespace = {}
                exec(data, namespace)  # execute with isolated scope

                hashfunc = namespace.get("trSitePasswordHash")
                if hashfunc:
                    self.passwdRequired = hashfunc("01", "01") is not None
                    self.passwordhashfunc = hashfunc

            if not self.passwdRequired:
                err, data = self.Transaction("monitor?q=loginscheme", None, "chklogins")
                if data and not err:
                    if data.get('validation', '').startswith("internal:"):
                        self.passwordhashfunc = trInternalPasswordHash
                        self.passwdRequired = True

        except Exception as err:
            # fallback to default password hash function
            if self.passwordhashfunc:
                self.passwdRequired = self.passwordhashfunc("01", "01") is not None
